...
async function fetchClassEnrollments(classIdentifier) {
    try {
        const response = await fetch(`${SERVER_URL}/api/enrollments/class/${encodeURIComponent(classIdentifier)}`);
...
async function loadClassFeedback(classIdentifier) {
    ...
    try {
        const response = await fetch(`${SERVER_URL}/api/classes/${classIdentifier}/feedback`);
