// login.js - Universal secure login handler for all 3 roles with timestamp and JWT token
...
function sanitize(input) {
  // Trim whitespace and remove MongoDB injection chars ($ and .), but preserve internal spaces
  if (typeof input !== 'string') return '';
  return input.trim().replace(/[$]/g, '').replace(/\./g, '');
  // Note: Removed \\s from regex to avoid stripping internal spaces; .trim() handles edges
}

function lockout() {
  isLocked = true;
  if (errorDiv) errorDiv.textContent = `Too many failed attempts. Please wait ${LOCKOUT_TIME} seconds.`;
...
if (loginForm) {
  loginForm.addEventListener('submit', async function(e) {
    ...
    const password = sanitize(loginForm.password.value); // Passwords rarely have spaces, but trim for safety
    ...
    try {
      ...
      if (response.ok && result.success) {
        // Store token securely with sessionStorage for tab isolation
        sessionStorage.setItem('token', result.token);
        // Add timestamp and token to user object for session management
        const authData = {
          ...result.user,
          timestamp: Date.now(), // CRITICAL: Add timestamp
          role: role,
          token: result.token // Include token for easy access in other scripts
        };
        // Store auth data with timestamp and token
        sessionStorage.setItem('authUser', JSON.stringify(authData));
        // NEW: Store role separately for easy checks in other scripts
        sessionStorage.setItem('role', role);
        // Also store individual fields if needed by other pages
        if (result.user.name) {
          sessionStorage.setItem('memberName', result.user.name);
        }
